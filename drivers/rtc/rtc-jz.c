...
{
	struct jz_rtc *rtc = dev_get_drvdata(dev);
	struct set_reg rtc_alarm_ms;
	unsigned int tmp;
	unsigned int rtc_time_ms;
	unsigned int alarm_is_enable;
	int ret = 0;
	unsigned long flags;
	switch (cmd) {
		case RTC_AIE_OFF:
		...
		case INGENIC_CTRL_RTC_REG:
			...
			break;
		case INGENIC_RTC_READ_TIME_MS:
			jzrtc_readl(rtc, RTC_RTCSR);
			rtc_time_ms = jzrtc_readl(rtc, RTC_HRCR);
			rtc_time_ms >>= 16;
			if (copy_to_user((void *)arg, &rtc_time_ms, sizeof(int))) {
				dev_err(dev, "Read rtc reg copy to user failed\n");
				return  -EFAULT;
			}else{
				return 0;
			}
		case INGENIC_RTC_ALARM_MS:
			{
				if (copy_from_user(&rtc_alarm_ms, (void *)arg, sizeof(struct set_reg))) {
					dev_err(dev, "set rtc wkup mode failed\n");
					ret = -EFAULT;
				}
				if(rtc_alarm_ms.ctrl == RTC_REG_CTRLW){
					jzrtc_writel(rtc, RTC_MSAR, rtc_alarm_ms.val & 0xffff);
				}else if(rtc_alarm_ms.ctrl == RTC_REG_CTRLR){
					rtc_alarm_ms.val = jzrtc_readl(rtc, RTC_MSAR);
				}
				if (copy_to_user((void *)arg, &rtc_alarm_ms, sizeof(struct set_reg))) {
					dev_err(dev, "Read rtc reg copy to user failed\n");
					return  -EFAULT;
				}else{
					return 0;
		...
		case INGENIC_IS_ALARM_ENABLE:
			if (copy_from_user(&alarm_is_enable, (void *)arg, sizeof(int))) {
				dev_err(dev, "set rtc ms enable failed\n");
				ret = -EFAULT;
			}
			tmp = jzrtc_readl(rtc, RTC_RTCCR);
			if(1 == alarm_is_enable){
				tmp &= ~RTCCR_AF;
				tmp |= (RTCCR_MS_EN);
			}if(2 == alarm_is_enable){
				tmp &= ~RTCCR_AF;
				tmp |= (RTCCR_AIE | RTCCR_AE | RTCCR_MS_EN);
			}else{
				tmp &= ~RTCCR_MS_EN;
			}
			jzrtc_writel(rtc, RTC_RTCCR, tmp);
			return 0;
		default:
			dev_err(dev, "RTC cmd not supported\n");
			return -ENOIOCTLCMD;
	}
	return -ENOIOCTLCMD;
}
...
static void jz_rtc_enable(struct jz_rtc *rtc)
{
	 ...
	 */
#if defined(CONFIG_SOC_PRJ008)||defined(CONFIG_SOC_PRJ007) && (defined (CONFIG_PM) || defined (CONFIG_ZERATUL))
	unsigned int reg = 0;
...
#ifndef CONFIG_SOC_PRJ007
	/*RTC initialization clears the millisecond enable by default to prevent unknown (X) states.*/
	jzrtc_writel(rtc, RTC_RTCCR, jzrtc_readl(rtc, RTC_RTCCR) & ~RTCCR_MS_EN);
#endif
