...
static int __init nand_init(void)
{
	...
	bch_div_clk = clk_get(NULL,nand_bch_get_div_clk_name());
	clk_enable(bch_clk);
        clk_disable(bch_div_clk);
        clk_set_rate(bch_div_clk, clk_get_rate(bch_div_clk));
	clk_enable(bch_div_clk);
	printk("clkgate=0x%08x bchclk=%ld \n",*(volatile unsigned int *)0xb0000020,clk_get_rate(bch_div_clk));
	for(i=0;i<8;i++)
		set_gpio_function(i,0,0); // A
	set_gpio_function(18,0,0); //group A
	set_gpio_function(19,0,0); //group A
