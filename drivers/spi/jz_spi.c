...
static int jz_spi_dma_txrx(struct spi_device *spi, struct spi_transfer *t)
{
	...
	/* cs is low */
	*(volatile unsigned int*)0xb0012048 = 0x8;
	start_transmit(hw);
	if(t->cs_change){
		finish_transmit(hw);
	}
	...
	if (!(hw->rw_mode & R_MODE)) {
		...
		if(t->cs_change){
			udelay(2000);
			*(volatile unsigned int*)0xb0012044 = 0x8;
		}
		flush_rxfifo(hw);
		clear_errors(hw);
...
{
	/*set cs is output1 */
	*(volatile unsigned int*)0xb0012018 = 0x8;
	*(volatile unsigned int*)0xb0012024 = 0x8;
	*(volatile unsigned int*)0xb0012038 = 0x8;
	*(volatile unsigned int*)0xb0012044 = 0x8;
