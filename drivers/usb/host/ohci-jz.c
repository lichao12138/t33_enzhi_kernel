...
static void jz_start_ohc(struct jz_ohci *jz_ohci)
{
	...
	clk_set_rate(jz_ohci->clk_div,48000000);
	clk_enable(jz_ohci->clk_div);
	cpm_start_ohci();
}
			  ...
			  struct platform_device *pdev)
{
	...
	jz_ohci->clk_div = clk_get(&pdev->dev, "div_uhc");
	if (IS_ERR(jz_ohci->clk_div)) {
		dev_err(&pdev->dev, "clk div get error\n");
		retval = PTR_ERR(jz_ohci->clk_div);
		goto err2;
	}
