...
__attribute__((__unused__)) static void jzmac_dump_skb_data(struct sk_buff *skb) {
	pr_debug("\n\n===================================\n");
	pr_debug("head = 0x%08x, data = 0x%08x, tail = 0x%08x, end = 0x%08x\n",
...
static int jz_mac_open(struct net_device *dev)
{
	...
	retval = phy_read(lp->phydev, MII_BMCR);
	phy_write(lp->phydev, MII_BMCR, (retval | BMCR_RESET));
...
s32 jz_mii_quest_phymode(synopGMACdevice * gmacdev) {
	int status = 0, ret = 0;
	unsigned int phy_id_hi = 0, phy_id_low = 0, phy_id = 0;
	unsigned int data = 0;

	status = synopGMAC_read_phy_reg(gmacdev,gmacdev->PhyBase,PHY_ID_HI_REG, &data);
	phy_id_low = data; /* OUI 3 to 18 bit */
	status = synopGMAC_read_phy_reg(gmacdev,gmacdev->PhyBase,PHY_ID_LOW_REG, &data);
	phy_id_hi = ((data >> 10) & 0x3f); /* OUI 19 to 24 bit */
	phy_id = phy_id_hi << 16 | phy_id_low;
	printk("phy devid = 0x%x\n", phy_id);

	if(phy_id == 0x282000) { /* ti tp83825 */
		phytype = 0;
		synopGMAC_write_phy_reg(gmacdev,gmacdev->PhyBase, 0x1e, 0x1f);
		synopGMAC_write_phy_reg(gmacdev,gmacdev->PhyBase, 0x1f, 0x467);//db83825 extreg 0x467
		synopGMAC_write_phy_reg(gmacdev,gmacdev->PhyBase, 0x1e, 0x401f);
		synopGMAC_read_phy_reg(gmacdev,gmacdev->PhyBase,0x1f, &data);
		printk("tp83825 reg-0x467 = 0x%x\n", data);
		if((data & 0x8) == 0) {
			//ret = 1;	//slave mode -- 25M
		}
	} else if(phy_id == 0x0 && (data == 0x128 || data == 0x118)) {
		printk("sz18201\n");
		phytype = 1;
	}
	return ret;
...
static int  jz_mii_bus_probe(struct platform_device *pdev)
{
	...
	int phyis25m = jz_mii_quest_phymode(gmacdev);
	if(phyis25m) {
...
static ssize_t mdio_cmd_set(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	struct platform_device *p_dev  =g_pdev;
struct net_device *n_dev;
struct jz_mac_local *mac_local;
n_dev =(struct net_device *) platform_get_drvdata(p_dev);
mac_local = netdev_priv(n_dev);
	if(!buf)
		return -ENOMEM;
	...
	if (!strncmp(buf, "r reg", sizeof("r reg")-1)) {
		...
		if (!ret)
			...
			sprintf(mdio_cmd_buf, "%s\n", "nok");
	} else if(!strncmp(buf, "reset", sizeof("reset")-1)){
		printk(KERN_ERR"gmac resek start\n");
		jzmac_reinit_locked(mac_local);
		printk(KERN_ERR"gmac resek end\n");
	}else if(!strncmp(buf, "getphy", sizeof("getphy")-1)){
		if(phytype >= 0)
			sprintf(mdio_cmd_buf, "phytype:%d", phytype);
		else
			sprintf(mdio_cmd_buf, "null", phytype);
	} else {
