...
static void dump_dma_desc(struct jzdma_channel *dmac)
{
	...
	dev_info(chan2dev(&dmac->chan), "CH_DSA(0x00) = 0x%08x\n",
		 readl(dmac->iomem + CH_DSA));
	dev_info(chan2dev(&dmac->chan), "CH_DTA(0x04) = 0x%08x\n",
		 readl(dmac->iomem + CH_DTA));
	dev_info(chan2dev(&dmac->chan), "CH_DTC(0x08) = 0x%08x\n",
		 readl(dmac->iomem + CH_DTC));
	dev_info(chan2dev(&dmac->chan), "CH_DRT(0x0c) = 0x%08x\n",
		 readl(dmac->iomem + CH_DRT));
	dev_info(chan2dev(&dmac->chan), "CH_DCS(0x10) = 0x%08x\n",
		 readl(dmac->iomem + CH_DCS));
	dev_info(chan2dev(&dmac->chan), "CH_DCM(0x14) = 0x%08x\n",
		 readl(dmac->iomem + CH_DCM));
	dev_info(chan2dev(&dmac->chan), "CH_DDA(0x18) = 0x%08x\n",
		 readl(dmac->iomem + CH_DDA));
	dev_info(chan2dev(&dmac->chan), "CH_DSD(0x1c) = 0x%08x\n",
		...
		enum dma_transfer_direction direction)
{
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		if(dmac->type == JZDMA_REQ_AES0 || dmac->type == JZDMA_REQ_AES1) {
			/* for aes, tsz is 111 just for RDIL */
			dcm |= DCM_TSZ_MSK;
			dcm |= DCM_RDIL_8;
			tsz = sg_dma_len(sg);
		} else {
