...
{
	u32 off0, off_set0, off_clr0;
	u32 off1, off_set1, off_clr1;
	int shift;
	int port = gpio / 32;
	...
	switch (lvl) {
		case DS_1_MODE:
			writel(1 << shift, chip->reg + off_clr1);
			writel(1 << shift, chip->reg + off_clr0);
			if(port == 0 && (shift >=12 && shift <=22)) {
				printk("now drive_strength use mode 1,typical values is 3.0mA\n");
			}else {
				printk("now drive_strength use mode 1,typical values is 9.8mA\n");
			}
			break;
		case DS_2_MODE:
			writel(1 << shift, chip->reg + off_clr1);
			writel(1 << shift, chip->reg + off_set0);
			if(port == 0 && (shift >=12 && shift <=22)) {
				printk("now drive_strength use mode 2,typical values is 6.1mA\n");
			}else {
				printk("now drive_strength use mode 2,typical values is 19.7mA\n");
			}
			break;
		case DS_3_MODE:
			writel(1 << shift, chip->reg + off_set1);
			writel(1 << shift, chip->reg + off_clr0);
			if(port == 0 && (shift >=12 && shift <=22)) {
				printk("now drive_strength use mode 3,typical values is 9.2mA\n");
			}else {
				printk("now drive_strength use mode 3,typical values is 29.7mA\n");
			}
			break;
		case DS_4_MODE:
			writel(1 << shift, chip->reg + off_set1);
			writel(1 << shift, chip->reg + off_set0);
			if(port == 0 && (shift >=12 && shift <=22)) {
				printk("now drive_strength use mode 4,typical values is 12.2mA\n");
			}else {
				printk("now drive_strength use mode 4,typical values is 39.5mA\n");
			}
			break;
		default:
			printk("Error: Unknown drive strength mode %d,so we use the default mode 1\n", lvl);
			writel(1 << shift, chip->reg + off_clr1);
			writel(1 << shift, chip->reg + off_clr0);
			break;
	}
...
EXPORT_SYMBOL(jz_gpio_set_drive_strength);
