...
struct ingenic_clk_reg_sleep clk_reg_sleep[] = {
	...
	REG_SLEEP(CPM_RISCV,		29, 28, 27, 0),
	...
	REG_SLEEP(CPM_SFC,		29, 28, 27, 0),
	REG_SLEEP(CPM_SFC1,		29, 28, 27, 0),
	REG_SLEEP(CPM_MSC0CDR,		29, 28, 27, 0),
	REG_SLEEP(CPM_MSC1CDR,          29, 28, 27, 0),
	REG_SLEEP(CPM_SSI,		29, 28, 27, 0),
	REG_SLEEP(CPM_SSI_SLV,		29, 28, 27, 0),
	REG_SLEEP(CPM_CIMCDR,		0, 0, 0, 0),
	REG_SLEEP(CPM_ISPMCDR,		29, 28, 27, 0),
	REG_SLEEP(CPM_ISPACDR,		29, 28, 27, 0),
	REG_SLEEP(CPM_TNPU,		29, 28, 27, 0),
	REG_SLEEP(CPM_PWM,		29, 28, 27, 0),
	REG_SLEEP(CPM_LDC,		29, 28, 27, 0),
	REG_SLEEP(CPM_I2SSPKCDR,	0,  0,  0, 0),
	REG_SLEEP(CPM_I2SSPKCDR1,	0,  0,  0, 0),
	REG_SLEEP(CPM_I2SMICCDR,	0,  0,  0, 0),
	REG_SLEEP(CPM_I2SMICCDR1,	0,  0,  0, 0),
...
static void clk_resume(void)
{
	...
	for (; num_regs > 0; --num_regs, ++resume_clk) {
		if(!resume_clk->ce && !resume_clk->busy) {
			...
			if(resume_clk->offset == CPM_ISPACDR || resume_clk->offset == CPM_ISPMCDR || resume_clk->offset == CPM_TNPU){
				if(resume_clk->value & (1 << resume_clk->stop)){
					resume_clk->value &= ~(1 << resume_clk->stop);
				}
			}
...
void clk_disable(struct clk *clk)
{
    ...
    if((strcmp(clk->name, "apll") == 0) || (strcmp(clk->name, "sclka") == 0) || (strcmp(clk->name, "mpll") == 0)){
		return;
    }
	if(clk->count == 0)
		return;
...
static ssize_t clock_set(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if(copy_from_user(buf, buffer, count)) {
		kfree(buf);
		printk("**********use help**********\n");
		printk("echo pll_set sclka 1400000000 > /proc/jz/clock/clocks, to set apll/mpll\n");
		printk("echo pll_get sclka > /proc/jz/clock/clocks, to set apll/mpll\n");
		return EFAULT;
	}
