 ...
 *			1	, x		  INVALID
 */
	[SELECTOR_B].route	= {CLK(SCLKA),CLK(MPLL),CLK(INVALID),CLK(INVALID)},
/*
 *		   bit31,bit30
 *			0	, 0		  SCLKA
 *			0	, 1		  MPLL
 ...
 *			1	, 1		  INVALID
 */
	[SELECTOR_C].route = {CLK(SCLKA) ,CLK(MPLL),CLK(INVALID),CLK(INVALID)},
/*
 *		   bit31,bit30
 *			0	, 0		  SCLKA
 *			0	, 1		  MPLL
 ...
 *			1	, 1		  EXT1
 */
	[SELECTOR_D].route = {CLK(SCLKA),CLK(MPLL),CLK(INVALID),CLK(EXT1)},
#undef CLK
};
...
static unsigned long div_get_rate(struct clk *clk)
{
	...
	if (!(strcmp(clk->name, "div_i2s_spk")) || !(strcmp(clk->name, "div_i2s_mic")) || !(strcmp(clk->name, "div_cim"))) {
		int m = 0, n = 0;
...
unsigned int jz_gcd(unsigned int a,unsigned int b) {
   unsigned int temp;
    while (b != 0) {
        temp = b;
        b = a % b;
...
static int div_set_rate(struct clk *clk, unsigned long rate)
{
	...
	int ce,stop,busy;
	unsigned int m = 0, n = 0 , g = 0;
	unsigned int reg_val, mask;
	unsigned long flags;

	if (!(strcmp(clk->name, "div_i2s_spk")) || !(strcmp(clk->name, "div_i2s_mic")) || !(strcmp(clk->name, "div_cim"))) {
		if(rate > 100000000 || !(clk->parent->rate) || !rate){
			printk("Rate must be less than 100M and greater than 0\n");
			return -1;
		}

		g = jz_gcd(clk->parent->rate, rate);
		m = rate / g;
		n = clk->parent->rate / g;


		if(!(strcmp(clk->name, "div_cim")))
		{
			if((20 * (m - 1) < n) && (m <= 0x1ff) && (n <= 0xfffff) && (clk->parent->rate / n == rate / m)){
				//printk("pll:%ld  rate:%ld  n:%d  m:%d\n",clk->parent->rate,rate,n,m);
			} else {
				printk("cim clk is not ok!\n");
				return -1;
			}
		} else {
			if((m <= 0x1ff) && (n <= 0xfffff) && (clk->parent->rate / n == rate / m)){
				//printk("pll:%ld  rate:%ld  n:%d  m:%d\n",clk->parent->rate,rate,n,m);
			} else {
				printk("spk/mic clk is not ok! now we switch clock source\n");
				return -1;
			}
		}
		reg_val = cpm_inl(div_clks[no].off) & 0xe0000000;
		reg_val |= (m << 20) | (n << 0);
		cpm_outl(reg_val,div_clks[no].off);
		clk->rate = (u64)(clk->parent->rate) / 1000 * m;
		clk->rate = clk->rate / n;
		clk->rate = clk->rate * 1000;
		/*printk("%s, parent = %ld, rate = %ld, n = %d, reg val = 0x%08x\n",
				__func__, clk->parent->rate, rate, n, cpm_inl(div_clks[no].off));*/
		return 0;
	}
	spin_lock_irqsave(&cpm_div_lock,flags);
	if(!(strcmp(clk->name, "div_sfc0")) || !(strcmp(clk->name, "div_sfc1")))
...
static int div_set_parent(struct clk *clk, struct clk *parent)
{
	...
	if ((div_clks[no].off == CPM_CIMCDR) || (div_clks[no].off == CPM_I2SSPKCDR) || (div_clks[no].off == CPM_I2SMICCDR) )
	{
