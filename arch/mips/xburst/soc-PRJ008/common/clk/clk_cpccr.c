...
#ifdef USE_PLL
		{
			unsigned int ddrdiv = cpm_inl(CPM_DDRCDR);
			unsigned int ddrsel = ddrdiv >> 30;
			...
			if(get_clk_from_id(CLK_ID_DIV_DDR)->parent) {
				ddr_pll_rate = clk_get_rate(get_clk_from_id(CLK_ID_DIV_DDR)->parent);
			} else {
				printk("ddr div clk set failure!\n");
				BUG_ON(1);
			}
