...
#define ZROOT_SIGNATURE		(0x002b3c)
...
void jz_wdt_restart(char *command)
{
	...
	}else if(((command != NULL) && !strcmp(command, "zrootfs"))) {
		while(cpm_inl(CPM_CPSPR) != ZROOT_SIGNATURE) {
			printk("set ZROOT_SIGNATURE\n");
			cpm_outl(0x5a5a, CPM_CPSPPR);
			cpm_outl(ZROOT_SIGNATURE, CPM_CPSPR);
			cpm_outl(0x0, CPM_CPSPPR);
			udelay(100);
		}
	}else {
