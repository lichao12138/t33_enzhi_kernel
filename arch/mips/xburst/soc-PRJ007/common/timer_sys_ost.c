...
static	void tmr_src_suspend(struct clocksource *cs)
{
	...
	tmr->pm.val_tccr = ost_readl(OST_TCCR);
	tmr->pm.val_ter = ost_readl(OST_TER);
	tmr->pm.val_tesr = ost_readl(OST_TESR);
	tmr->pm.val_tecr = ost_readl(OST_TECR);
	tmr->pm.val_tcr = ost_readl(OST_TCR);
	tmr->pm.val_tfr = ost_readl(OST_TFR);
	tmr->pm.val_tmr = ost_readl(OST_TMR);
	tmr->pm.val_t1dfr = ost_readl(OST_T1DFR);
...
static void tmr_src_resume(struct clocksource *cs)
{
	...
	ost_writel(OST_TER, tmr->pm.val_ter);
	ost_writel(OST_TESR, tmr->pm.val_tesr|(1 << CLKSOURCE_CH));
	ost_writel(OST_TECR, tmr->pm.val_tecr);
	ost_writel(OST_TCR, tmr->pm.val_tcr | (1<<CLKEVENT_CH));
	ost_writel(OST_TFR, tmr->pm.val_tfr);
	ost_writel(OST_TMR, tmr->pm.val_tmr | (1 << CLKEVENT_CH));
	ost_writel(OST_T1DFR, tmr->pm.val_t1dfr);
	ost_writel(OST_TCCR, tmr->pm.val_tccr);
#endif/*CONFIG_PM*/
}
...
static inline void rst_allchn(void) {
	ost_writel(OST_TMR , ((3 << CLKEVENT_CH)));
	ost_writel(OST_TFR , (~(3 << CLKEVENT_CH)));
	ost_writel(OST_TCR , (3 << CLKEVENT_CH));//OS Timer clear Register
	ost_writel(OST_TESR , (3 << CLKEVENT_CH));
	ost_writel(OST_TMR , (~(3 << CLKEVENT_CH)));
}
